...
3. **Confirm a clean baseline**: `./gradlew spotlessCheck test`.
...
```bash
./gradlew spotlessCheck          # Formatting (googleJavaFormat + ktlint)
...
- Fix Spotless failures with `./gradlew spotlessApply` — do not hand-format or suppress the check.
...
- Run `./gradlew test` before every commit (also runs automatically on `git push` via lefthook).

## 6. Code Coverage

- Coverage is measured with JaCoCo (`./gradlew jacocoTestReport`, report at
