...
@Tag(
        ...
        description = "Submit prior-authority requests to the Legal Aid Agency, and save/resume in-progress drafts"
)
@RestController
@CrossOrigin(origins = "*")
...
public class PriorAuthorityController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Draft created. Location header contains the draft URL."),
            @ApiResponse(responseCode = "400", description = "applicationId missing.", content = @Content)
    })
    @PostMapping("/drafts")
    public ResponseEntity<DraftIdResponse> createDraft(@Valid @RequestBody PriorAuthorityDraft draft) {
