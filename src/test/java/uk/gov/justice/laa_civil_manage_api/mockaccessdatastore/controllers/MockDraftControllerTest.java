...
class MockDraftControllerTest {
  ...
  void createReturns201WithDraftId() throws Exception {
    mockMvc
        .perform(
            post("/mock-access-data-store/drafts")
  ...
  void updateAfterCreateReturns200AndReplacesBody() throws Exception {
    ...
    mockMvc
        .perform(
            put("/mock-access-data-store/drafts/{draftId}", draftId)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            get("/mock-access-data-store/drafts")
  ...
  void updateReturns404WhenDraftDoesNotExist() throws Exception {
    mockMvc
        .perform(
            put("/mock-access-data-store/drafts/{draftId}", UUID.randomUUID())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void getByDraftIdSucceedsWhenIdIsPresent() throws Exception {
    ...
    mockMvc
        .perform(get("/mock-access-data-store/drafts/{draftId}", draftId))
        .andExpect(status().isOk())
  ...
  void getByDraftIdReturns404WhenIdIsNotPresent() throws Exception {
    mockMvc
        .perform(get("/mock-access-data-store/drafts/{draftId}", UUID.randomUUID()))
        .andExpect(status().isNotFound());
  }
  ...
  void deleteReturns204AndRemovesDraft() throws Exception {
    ...
    mockMvc
        .perform(delete("/mock-access-data-store/drafts/{draftId}", draftId))
